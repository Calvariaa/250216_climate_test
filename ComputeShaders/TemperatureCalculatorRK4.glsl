...
void main(){
    ...
    float T_k2 = T0 + 0.5 * dt * k1;
    float k2_temp_left = imageLoad(temperature_readonly, ivec2(left_uv)).r + 0.5 * dt * k1;
    float k2_temp_right = imageLoad(temperature_readonly, ivec2(right_uv)).r + 0.5 * dt * k1;
    float k2_temp_bottom = imageLoad(temperature_readonly, ivec2(bottom_uv)).r + 0.5 * dt * k1;
    float k2_temp_top = imageLoad(temperature_readonly, ivec2(top_uv)).r + 0.5 * dt * k1;
    float k2 = computeHeatEquation(id, T_k2, k2_temp_left, k2_temp_right, k2_temp_bottom, k2_temp_top);
    
    float T_k3 = T0 + 0.5 * dt * k2;
    float k3_temp_left = imageLoad(temperature_readonly, ivec2(left_uv)).r + 0.5 * dt * k2;
    float k3_temp_right = imageLoad(temperature_readonly, ivec2(right_uv)).r + 0.5 * dt * k2;
    float k3_temp_bottom = imageLoad(temperature_readonly, ivec2(bottom_uv)).r + 0.5 * dt * k2;
    float k3_temp_top = imageLoad(temperature_readonly, ivec2(top_uv)).r + 0.5 * dt * k2;
    ...
    float final_temp = T0 + dt * (k1 + 2.0 * k2 + 2.0 * k3 + k4) / 6.0;
